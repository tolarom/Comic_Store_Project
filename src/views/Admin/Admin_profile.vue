<template>
  ...
  <div class="min-h-screen bg-gray-50 py-16 px-4 sm:px-6 lg:px-8 ml-[250px]">
    <div class="max-w-5xl mx-auto">
      <h1 class="text-4xl font-bold text-blue-600 mb-12">Admin Profile</h1>
      ...
      <div v-else class="grid md:grid-cols-2 gap-10 items-start">
        ...
        <div class="bg-white rounded-2xl shadow-xl p-10">
          <h2 class="text-2xl font-bold text-blue-600 mb-8">Profile Information</h2>

          <div class="space-y-6 text-lg">
            <div class="flex justify-between py-3 border-b border-gray-300">
              <span class="font-medium text-gray-600">Full Name:</span>
              <span class="font-semibold">{{ fullUserData?.full_name || 'Not provided' }}</span>
            </div>
            <div class="flex justify-between py-3 border-b border-gray-300">
              <span class="font-medium text-gray-600">Username:</span>
              <span class="font-semibold">{{ fullUserData?.username || 'Not provided' }}</span>
            </div>
            <div class="flex justify-between py-3 border-b border-gray-300">
              <span class="font-medium text-gray-600">Email:</span>
              <span class="font-semibold">{{ fullUserData?.email || 'Not provided' }}</span>
            </div>
            <div class="flex justify-between py-3 border-b border-gray-300">
            ...
            <div class="flex justify-between py-3 border-t border-gray-300">
              ...
              <span class="font-semibold capitalize px-3 py-1 bg-blue-100 text-blue-800 rounded-full">{{ fullUserData?.role || 'user' }}</span>
            </div>
          </div>
        </div>
      </div>
...
const goToEditProfile = () => {
  router.push({ name: 'EditAdminProfile' })
}
